...
def test_find_element(driver, fn):
    """ Test a find_element_by_FOO method of both the webdriver
        ...
        a WebElement.
    """
    element = getattr(driver, fn)('div')
    assert(isinstance(element, WebElement))
    assert(element.id == TestServer.TEST_FIND_ELEMENT)
    child = getattr(element, fn)('div')
...
if __name__ == '__main__':
    ...
    assert(TestServer.TEST_URL in driver.current_url)
    driver.back()
    driver.forward()
    driver.refresh()
    ...
    assert(TestServer.TEST_ASYNC_EXECUTE_RETURN_VALUE == driver.execute_async_script(TestServer.TEST_EXECUTE_ASYNC_SCRIPT))
    assert(TestServer.TEST_ASYNC_EXECUTE_RETURN_VALUE == driver.execute_async_script(TestServer.TEST_EXECUTE_ASYNC_SCRIPT,
                                                                               TestServer.TEST_EXECUTE_SCRIPT_ARGS))
